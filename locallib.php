...
/**
 * Returns moodleoverflow forum posts tagged with a specified tag.
 *
 * This is a callback used by the tag area mod_moodleoverflow/moodleoverflow_posts to search for moodleoverflow forum posts
 ...
 */
function mod_moodleoverflow_get_tagged_posts($tag, $exclusivemode = false, $fromctx = 0, $ctx = 0, $rec = 1, $page = 0) {
    global $OUTPUT;
    ...
    $query = "SELECT p.*, d.*,
                ...
                JOIN {modules} m ON m.name='moodleoverflow'
                JOIN {course_modules} cm ON cm.module = m.id AND cm.instance = mo.id
                JOIN {tag_instance} tt ON p.id = tt.itemid
                JOIN {course} c ON cm.course = c.id
                JOIN {context} ctx ON ctx.instanceid = cm.id AND ctx.contextlevel = :coursemodulecontextlevel
               WHERE tt.itemtype = :itemtype AND tt.tagid = :tagid AND tt.component = :component
                 AND cm.deletioninprogress = 0
                 AND p.id %ITEMFILTER% AND c.id %COURSEFILTER%";

    $params = array('itemtype' => 'moodleoverflow_posts', 'tagid' => $tag->id, 'component' => 'mod_moodleoverflow',
    ...
    $query .= ' c.sortorder, cm.id, p.id';

    $totalpages = $page + 1;

    // Use core_tag_index_builder to build and filter the list of items.
    $builder = new core_tag_index_builder('mod_moodleoverflow', 'moodleoverflow_posts', $query, $params, $page * $perpage, $perpage + 1);
    while ($item = $builder->has_item_that_needs_access_check()) {
        ...
        $builder->walk(function ($taggeditem) use ($courseid, $modinfo, $builder, $item) {
            if ($taggeditem->courseid == $courseid) {
                ...
                $moodleoverflow = (object)['id' => $taggeditem->moodleoverflow,
                ...
                $accessible = moodleoverflow_user_can_see_post($moodleoverflow, $discussion, $post, null, $cm);
    ...
    if ($items) {
        ...
        foreach ($items as $item) {
            ...
            $pageurl = new moodle_url('/mod/moodleoverflow/discussion.php', array('d' => $item->discussion), 'p' . $item->id);
        ...
        return new core_tag\output\tagindex($tag, 'mod_moodleoverflow', 'moodleoverflow_posts', $content,
